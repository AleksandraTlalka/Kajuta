...
Obstruction::Obstruction(float x, float yHole, sf::Texture pTexture, Game* pGame)
	:GraphicObject({ x,0 }, { 0,0 }, pTexture, pGame),
	point(1)
{
	pipes.push_back(new Pipe({ x, 0 }, { PIPE_WIDTH,PIPE_HEIGHT }, pTexture, pGame, pipe_type::pipe_top));
	pipes[0]->setPosition({ pipes[0]->getPosition().x, -PIPE_HEIGHT + yHole });
	pipes.push_back(new Pipe({ x, 0 }, { PIPE_WIDTH,PIPE_HEIGHT }, pTexture, pGame, pipe_type::pipe_bottom));
